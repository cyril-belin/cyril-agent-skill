...
- To disable any active stop enforcement for a session, delete the loop marker file (`.agents/state/loops-nuxt.active` by default).
...
This skill is agent-neutral. It assumes only that the agent can read files, run shell commands, search the codebase, invoke other skills, and use MCP tools if configured. Agent-specific lifecycle enforcement (such as a Stop hook) belongs in `integrations/<agent>/loops-nuxt/` and is optional.
